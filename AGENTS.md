...
- Chromium uses the `favicon` permission and `/_favicon/` endpoint for missing or inaccessible native page icons. Live and recently closed native icons are marked with `data-native-favicon` and rendered as `currentColor` masks; Firefox retains upstream favicon handling.
